...
namespace FIMSpace
{
    ...
    public partial class LeaningProcessor
    {
        ...
        private AnimatorControllerParameterType _grndType = AnimatorControllerParameterType.Bool;
        ...
        [Tooltip( "'IsMoving' Animator property value which needs to be exceeded in order to define current character movement state as moving, below this value - stopping" )]
        [HideInInspector] public float AccelValueThreshold = 0.25f;
        ...
        public void RefreshAnimiatorParams()
        {
            ...
            if( _hash_grnd != -1 )
            {
                ...
                {
                    var param = unityanim.GetParameter( i );
                    if( param.nameHash == _hash_grnd ) _grndType = ( param.type );
                }
            }
        }
        ...
        private void UpdateStack()
        {
                ...
                if( unityanim )
                {
                    if( _hash_acc != -1 )
                    {
                        if( _accType == AnimatorControllerParameterType.Bool )
                            ...
                            IsCharacterAccelerating = unityanim.GetFloat( _hash_acc ) > AccelValueThreshold;
            ...
            if( unityanim )
            {
                if( !TryAutoDetectGround )
                    if( _hash_grnd != -1 )
                    {
                        if( _grndType == AnimatorControllerParameterType.Bool )
                            IsCharacterGrounded = unityanim.GetBool( _hash_grnd );
                        else if( _grndType == AnimatorControllerParameterType.Float )
                            IsCharacterGrounded = unityanim.GetFloat( _hash_grnd ) > 0.999f;
                        else if( _grndType == AnimatorControllerParameterType.Int )
                            IsCharacterGrounded = unityanim.GetInteger( _hash_grnd ) > 0;
                    }
