    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void Move()
        {
            ...
            if (_input.move != Vector2.zero)
            {
                ...
                if (directBlendDelay > 1f) directBlendDelay = 1f;
            }
            else
            {
                directBlendDelay -= Time.deltaTime * 2f;
                if (directBlendDelay < -0.2f) directBlendDelay = -0.2f;
            }
            Vector3 targetDirecton = Quaternion.Euler(0.0f,_targetRotation,0.0f) * Vector3.forward;
            Vector3 directMovement = transform.forward;

            targetDirecton = Vector3.Lerp(targetDirecton, directMovement, DirectMoveBlend * directBlendDelay);
